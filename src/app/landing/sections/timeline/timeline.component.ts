...
@Component({
  ...
  template: `
    <section id="how-it-works" class="section overflow-hidden bg-white" aria-labelledby="how-heading">
      <div class="mx-auto max-w-7xl px-6">
        <p appReveal mode="fade" class="section-label">Cum funcționează</p>
        <h2 id="how-heading" appReveal mode="words" [stagger]="0.05" class="mt-6 max-w-2xl text-4xl font-bold tracking-tighter text-black sm:text-5xl">
      ...
      <div #pinWrap class="relative mt-16">
        <!-- Progress rail -->
        <div aria-hidden="true" class="absolute top-[4.4rem] right-0 left-0 hidden h-px bg-black/10 lg:block">
          <div #progress class="h-full origin-left scale-x-0 bg-black shadow-[0_0_12px_rgba(0,0,0,0.1)]"></div>
        </div>
        ...
        >
          @for (step of steps; track step.index; let i = $index) {
            <li
              ...
              mode="fade"
              [delay]="i * 0.08"
              class="card relative w-full shrink-0 rounded-3xl p-8 lg:w-[26rem] transition-transform hover:-translate-y-1"
            >
              <div class="mb-6 flex items-center gap-4">
                <span class="grid size-12 place-items-center rounded-2xl bg-black/5 text-black" [innerHTML]="step.icon | safeHtml" aria-hidden="true"></span>
                <span class="text-sm font-bold tracking-[0.2em] text-black/40">PASUL {{ step.index }}</span>
              </div>
              <h3 class="text-xl font-bold text-black">{{ step.title }}</h3>
              <p class="mt-3 leading-relaxed text-black/70">{{ step.description }}</p>
              @if (i < steps.length - 1) {
                <span aria-hidden="true" class="absolute top-1/2 -right-7 hidden -translate-y-1/2 text-black/20 lg:block">
  ...
  readonly steps: TimelineStep[] = [
    {
      ...
      description: 'Folosește camera telefonului într-o lumină bună. Păstrează imaginea clară și focalizată.',
    ...
    },
    {
      ...
      description: 'Fotografia este criptată și procesată privat. Nu este nevoie de un cont pentru a încerca.',
    ...
    },
    {
      index: '03',
      title: 'Analiza AI',
      description: 'O rețea neurală antrenată pe seturi de date clinice examinează forma, culoarea și textura.',
    ...
    {
      index: '04',
      title: 'Generare Rezultat',
      description: 'Primești o estimare clară a riscului cu detalii vizuale despre regiunile analizate.',
    ...
    },
    {
      index: '05',
      title: 'Recomandări',
      description: 'Dacă ceva pare suspect, SkinAlert îți recomandă evaluarea unui medic dermatolog.',
