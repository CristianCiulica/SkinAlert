        ...
        <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          @for (card of cards; track card.title; let i = $index) {
            <div
              ...
              mode="fade"
              [delay]="(i % 3) * 0.08"
              class="card group relative flex min-h-[14rem] flex-col justify-between rounded-2xl p-7"
            >
              <div class="flex items-start justify-between">
                <span
                  ...
                  aria-hidden="true"
                ></span>
                <span class="text-xs font-semibold tabular-nums text-ink/30">{{ card.index }}</span>
              </div>
              <div>
